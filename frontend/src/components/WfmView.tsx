...
import { useWfmMetrics } from '../lib/hooks/useWfmMetrics';
import type { ThresholdSet } from '../lib/useThresholds';
import type { ChartId, ReportLayout } from './ReportBuilder';
import type { CallRecord } from '../types/calls';
import { BarChart3 } from 'lucide-react';

const QueueChart = lazy(() => import('./Charts').then((m) => ({ default: m.QueueChart })));
...
interface Props {
  ...
  queueData: { name: string; value: number }[];
}

export function WfmView({ calls, thresholds, selectedCharts, onChartsChange, layout, onLayoutChange, queueData }: Props) {
  const { kpis } = useWfmMetrics(calls, thresholds);
  const gridClass = layout === '3' ? 'grid gap-6 md:grid-cols-2 xl:grid-cols-3' : 'grid gap-6 xl:grid-cols-2';

  return (
    <ModuleErrorBoundary moduleName="WFM">
      <div>
          ...
          <Suspense fallback={<ChartSkeleton />}>
            {selectedCharts.length === 0 ? (
              <div className="col-span-full flex flex-col items-center justify-center rounded-xl border-2 border-dashed border-slate-200 bg-white py-16 text-center dark:border-white/10 dark:bg-white/5">
                ...
                <button type="button" onClick={() => onChartsChange(['queues'])}
