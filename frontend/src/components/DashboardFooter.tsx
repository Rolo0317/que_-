...
export function DashboardFooter({ totalCalls, agentCount, slaPercent }: DashboardFooterProps) {
  ...
  return (
    <motion.footer
      ...
      className="relative mt-16 overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-panel
                 dark:border-que-teal/30 dark:bg-gradient-to-br dark:from-ink dark:via-slate-900 dark:to-slate-800"
    >
      {/* Fondo decorativo — visible solo en dark */}
      <div className="pointer-events-none absolute inset-0 opacity-0 dark:opacity-[0.06]">
      ...
      <div className="relative z-10 grid gap-8 p-8 md:grid-cols-[1.3fr_0.7fr] md:items-center">
        ...
        >
          <BrandLogo className="w-36 dark:brightness-110" />
          <p className="mt-4 max-w-md text-sm leading-6 text-slate-600 dark:text-white/70">
          ...
          <ul className="mt-5 space-y-2">
            {features.map((f) => (
              <li key={f} className="flex items-center gap-2 text-xs text-slate-500 dark:text-white/60">
      ...
      {/* Bottom bar */}
      <div className="relative z-10 flex flex-col gap-3 border-t border-slate-100 px-8 py-4 dark:border-white/5 sm:flex-row sm:items-center sm:justify-between">
        <p className="text-xs text-slate-400 dark:text-white/40">© 2026 QUE+ Dashboard. Todos los derechos reservados.</p>
        <div className="flex gap-5">
          {['Ayuda', 'GitHub', 'Soporte'].map((link) => (
            <a
              key={link}
              href="#"
              className="text-xs text-slate-400 transition hover:text-que-teal dark:text-white/40"
            >
              {link}
            </a>
          ))}
