import { Activity, BarChart3, PhoneCall, RefreshCw, Star, Users, TrendingUp, ShieldCheck } from 'lucide-react';
...
const HourlyChart = lazy(() =>
  import('./components/Charts').then((m) => ({ default: m.HourlyChart })),
);
const TypeMixChart = lazy(() =>
  import('./components/Charts').then((m) => ({ default: m.TypeMixChart })),
);
const AgentScoreChart = lazy(() =>
  import('./components/Charts').then((m) => ({ default: m.AgentScoreChart })),
);

const formatPercent = (v: number) => `${(v * 100).toFixed(1)}%`;
const formatDuration = (s: number) => `${Math.round(s)} s`;

// Helpers de status (semáforo)
...
const occupancyStatus = (v: number): KpiStatus =>
  v >= 0.75 && v <= 0.90 ? 'good' : v >= 0.60 && v <= 0.95 ? 'warning' : 'neutral';
...
function App() {
  ...
  const wfmKpis = useMemo(() => [
    {
      label: 'Ocupacion',
      value: formatPercent(metrics.occupancy),
      helper: 'Talk / disponible',
      target: 'Meta: 80–85%',
      status: occupancyStatus(metrics.occupancy),
    },
    {
      label: 'Utilizacion',
      ...
      target: 'Meta: >85%',
      status: pct(metrics.utilization, 0.85, 0.75),
    },
    {
      label: 'Shrinkage',
      value: formatPercent(metrics.shrinkage),
      helper: 'Fuera de produccion',
      target: 'Meta: <25%',
      status: pctInv(metrics.shrinkage, 0.25, 0.35),
    },
    {
      label: 'Adherencia',
      value: formatPercent(metrics.adherence),
      helper: 'Horario plan vs real',
      target: 'Meta: >95%',
      status: pct(metrics.adherence, 0.95, 0.90),
    },
    {
      label: 'Asistencia',
      ...
      target: 'Meta: >95%',
      status: pct(metrics.attendance, 0.95, 0.90),
    },
  ], [metrics]);
  ...
  const operacionesKpis = useMemo(() => [
    ...
    {
      label: 'SLA',
      value: formatPercent(metrics.serviceLevel),
      helper: 'Contestadas a tiempo',
      target: 'Meta: >80%',
      status: pct(metrics.serviceLevel, 0.80, 0.70),
    },
    {
      label: 'Abandono',
      ...
      helper: 'Sobre total',
      target: 'Meta: <5%',
      status: pctInv(metrics.abandonRate, 0.05, 0.10),
    },
    {
      label: 'ASA',
      value: formatDuration(metrics.avgSpeedAnswer),
      helper: 'Espera promedio',
      target: 'Meta: <30 s',
      status: secInv(metrics.avgSpeedAnswer, 30, 60),
    },
    {
      label: 'AHT / TMO',
      value: formatDuration(metrics.avgDuration),
      helper: 'Duracion media',
      status: 'neutral' as KpiStatus,
    },
  ], [metrics]);
  ...
  const calidadKpis = useMemo(() => [
    {
      label: 'FCR',
      value: formatPercent(metrics.firstContactResolution),
      helper: 'Resuelto primer contacto',
      target: 'Meta: >80%',
      status: pct(metrics.firstContactResolution, 0.80, 0.70),
    },
    {
      ...
      helper: 'Derivaciones',
      target: 'Meta: <10%',
      status: pctInv(metrics.transferRate, 0.10, 0.20),
    },
    {
      label: 'QA Score',
      value: metrics.avgQaScore.toFixed(1),
      helper: 'Calidad promedio (0–100)',
      target: 'Meta: >85',
      status: pct(metrics.avgQaScore / 100, 0.85, 0.75),
    },
    {
      label: 'Satisfaccion',
    ...
    <div className={`${theme === 'dark' ? 'dark' : ''}`}>
      <div className="relative min-h-screen bg-mist text-ink transition-colors dark:bg-[#07181c] dark:text-white">
        ...
        <main className="relative mx-auto max-w-7xl px-4 py-6 lg:ml-20 lg:px-8">
          ...
          {/* Tabs de módulos */}
          <div className="mt-6 flex items-center gap-1 rounded-xl border border-slate-200 bg-white p-1 shadow-sm dark:border-white/10 dark:bg-slate-900 sm:w-fit">
          ...
          {/* Grid de KPIs del módulo activo */}
          ...
          {/* Leyenda de semáforo */}
          <div className="mt-4 flex flex-wrap items-center gap-4 text-xs text-slate-500 dark:text-white/50">
            <span className="flex items-center gap-1.5">
              <span className="h-2 w-2 rounded-full bg-emerald-500" /> Cumple meta
            </span>
            <span className="flex items-center gap-1.5">
              <span className="h-2 w-2 rounded-full bg-amber-400" /> En riesgo
            </span>
            <span className="flex items-center gap-1.5">
              <span className="h-2 w-2 rounded-full bg-rose-500" /> Fuera de meta
            </span>
            <span className="flex items-center gap-1.5">
              <span className="h-2 w-2 rounded-full bg-que-teal" /> Referencial
            </span>
          </div>

          {/* Sección de gráficas + Arma tu informe */}
          <section className="mt-8 grid gap-6 lg:grid-cols-[260px_1fr]">
            <div className="space-y-4">
              ...
              <article className="rounded-lg border border-slate-200 bg-white p-4 shadow-panel dark:border-white/10 dark:bg-white/10">
                <div className="flex items-center gap-2 text-sm font-semibold text-ink dark:text-white">
                  <Star size={16} className="text-plus-orange" aria-hidden="true" />
                  Satisfaccion media
                </div>
                <p className="mt-2 text-4xl font-bold text-ink dark:text-white">{metrics.avgScore.toFixed(2)}</p>
                <p className="mt-1 text-xs text-slate-500 dark:text-white/60">Escala 1 a 5</p>
              </article>
            </div>
